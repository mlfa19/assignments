...
\includegraphics[width=\linewidth]{figures/titaniclogisticregression}\marginnote{To interpret these flow diagrams, think of the circles (also called nodes) as representing values that are either input to or computed by the network.  Directed lines (also called edges) represent data flowing in the network.  Each edge multiplies the value flowing into it by a weight (represented by a text label on the edge).}
